...
public class SonarNetworkManager
{
    ...
    private <MSG extends SonarMessage<T>, T> boolean processMessage(MSG msg, Supplier<NetworkEvent.Context> ctx)
    {
        try
        {
            msg.processPacket((T) (ctx.get().getDirection().getReceptionSide().isClient() ? this.clientMessageHandler.get().get() : this.serverMessageHandler.get().get()), new ForgeSonarNetworkContext(ctx));
        }
        catch (Exception e)
        {
