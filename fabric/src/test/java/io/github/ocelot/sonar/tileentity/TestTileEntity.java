...
public class TestTileEntity extends BlockEntity implements ValueContainer
{
    ...
    public void getEntries(Level world, BlockPos pos, List<ValueContainerEntry<?>> entries)
    {
        for (int i = 0; i < 1024; i++)
        {
            ...
            entries.add(new RegistryObjectValueContainerEntry<>(new TextComponent("Enchantment " + i), "enchantment" + i, Registry.ENCHANTMENT, Enchantments.BINDING_CURSE));
            entries.add(new RegistryObjectValueContainerEntry<>(new TextComponent("Block " + i), "block" + i, Registry.BLOCK, Blocks.ACACIA_PLANKS));
            entries.add(new RegistryObjectValueContainerEntry<>(new TextComponent("Sound Event " + i), "soundevent" + i, Registry.SOUND_EVENT, SoundEvents.BEEHIVE_DRIP));
